...
#define NULL_THREAD_ID -1L
    ...
    public:
        ...
        ///     to block in a fashion visible to ConcRT.
        /// </summary>
        LockQueueNode(Context * pContext, unsigned int ticket, unsigned int timeout = COOPERATIVE_TIMEOUT_INFINITE) 
            : m_pNextNode(NULL), m_pContext(pContext), m_ticketState(ticket), m_hTimer(NULL), m_trigger(NotTriggered), m_fTimedNodeInvalid(0)
        {
        }
    ...
    /// <summary>
    ///     Same as DispatchNodeTimeoutTimer, but used only for XP
    /// </summary>
    static void CALLBACK DispatchNodeTimeoutTimerXP(PVOID pContext, BOOLEAN timerOrWaitFired)
    {
        LockQueueNode *pNode = reinterpret_cast<LockQueueNode *> (pContext);
        ...
        for(;;)
        {
            if (!DeleteTimerQueueTimer(GetSharedTimerQueue(), pNode->m_hTimer, NULL))
            {
...
///     true if the lock is acquired, false otherwise
/// </returns>
_CRTIMP bool critical_section::try_lock_for(unsigned int timeout)
{
