...
extern "C" _CRTIMP int __BuildCatchObjectHelper(
...
extern "C" _CRTIMP void __BuildCatchObject(
    EHExceptionRecord *pExcept,         // Original exception thrown
    void *pRN,                          // This is a pointer to the object
                                        // that we want to build while doing
                                        // COM+ eh. If we are in our own eh,
...
extern "C" _CRTIMP void __cdecl __DestructExceptionObject(
    ...
    // _except_handler3)
    if (pExcept == NULL || !(PER_IS_MSVC_EH(pExcept)))
    {
        return;
    }
...
#pragma prefast(suppress:__WARNING_REDUNDANT_POINTER_TEST __WARNING_REDUNDANT_POINTER_TEST_FAR_EVIDENCE, "Current definition of PER_CODE ensures that pExcept cannot be NULL")
...
// has passed in.
//
_CRTIMP int __cdecl _is_exception_typeof(const type_info & type, struct _EXCEPTION_POINTERS * ep)
{
    DASSERT(ep != NULL);
