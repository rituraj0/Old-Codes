// Exercise. 16.07 : ex16_07.cpp
// Program to view/remove books from the shopping cart.
#include <iostream>
...
int main()
{
   ...
   if ( getenv( "CONTENT_LENGTH" ) ) {
	  ...
	  if( addString == "false" ) {
		  ...
		  int x = cookieString.find( isbnEntered );
   ...
   // cookie data exists
   if ( addString == "true" && cookieString != "" ) {
   ...
   // book removed
   if ( addString == "false" ) {
	  cout << "Item has been removed from the "
		  << "shopping cart.<br/>";
      isbnEntered = "";
	  outputBooks( cookieString, isbnEntered );
   } // end if
...
void outputBooks( const string &cookieRef, 
				  const string &isbnRef )
{
   char book[ 50 ] = "";
   char year[ 50 ] = "";
   char isbn[ 50 ] = "";
