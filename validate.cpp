...
_CRTIMP BOOL __cdecl _ValidateRead( const void *data, UINT size )
{
        BOOL bValid = TRUE;
        if ( data == NULL ) {
...
_CRTIMP BOOL __cdecl _ValidateWrite( void *data, UINT size )
{
        BOOL bValid = TRUE;
        if ( data == NULL ) {
...
_CRTIMP BOOL __cdecl _ValidateExecute( FARPROC code )
{
        BOOL    bValid = TRUE;
        if ( code == NULL ) {
...
//
_CRTIMP void __cdecl EHTraceOutput(const char *format, ...)
{
    va_list arglist;
    char buf[1024];
...
//
_CRTIMP const char* __cdecl EHTraceIndent(int level)
{
...
//
_CRTIMP const char * __cdecl EHTraceFunc(const char *func)
{
...
//
_CRTIMP void __cdecl EHTracePushLevel(int new_level)
{
...
_CRTIMP void __cdecl EHTracePopLevel(bool restore)
{
    --__ehtrace_level_stack_depth;
    if (restore &&
...
//
_CRTIMP int __cdecl EHTraceExceptFilter(const char *func, int expr)
{
...
//
_CRTIMP void __cdecl EHTraceHandlerReturn(const char *func, int level, EXCEPTION_DISPOSITION result)
{
