...
namespace details
{
        ...
        m_lastServiceScan(0)
    {
        ...
        m_hThrottlingEvent = CreateEventW(NULL, FALSE, FALSE, NULL);
        if (m_hThrottlingEvent == NULL)
        {
            throw scheduler_resource_allocation_error(HRESULT_FROM_WIN32(GetLastError()));
        }
        ...
        if (!CreateTimerQueueTimer(&m_hThrottlingTimer, 
                                   NULL,
                                   SchedulerBase::ThrottlerDispatchBridge,
                                   this,
                                   PSEUDOINFINITE,
                                   PSEUDOINFINITE,
                                   WT_EXECUTEDEFAULT))
    ...
    /// </summary>
    void SchedulerBase::ThrottlerTrampoline(PVOID pData, BOOLEAN waitOrTimerFired)
    {
        ...
        if (delta < delay)
        {
            ...
            ChangeTimerQueueTimer(NULL, pScheduler->m_hThrottlingTimer, delay - delta, PSEUDOINFINITE);
        }
        else
    ...
    {
        OMTRACE(MTRACE_EVT_THROTTLERDISPATCH, this, NULL, NULL, GetTickCount());
        ...
        if (m_pendingDeferredCreates > 0)
        {
            ...
            for(;;)
            {
                ...
                OMTRACE(MTRACE_EVT_CREATEDTHROTTLEDCONTEXT, this, pContext, NULL, GetTickCount());
            ...
            if (fReschedule)
            {
                ...
                ChangeTimerQueueTimer(NULL, m_hThrottlingTimer, delay, PSEUDOINFINITE);
            }
        }
    }
    ...
    void SchedulerBase::DestroySchedulerEventHandlers()
    {
        ...
        for(;;)
        {
            if (!DeleteTimerQueueTimer(NULL, m_hThrottlingTimer, INVALID_HANDLE_VALUE))
            {
...
#pragma warning(default: 4702)
    ...
    void SchedulerBase::RemoveVirtualProcessors(IVirtualProcessorRoot **ppVirtualProcessorRoots, unsigned int count)
    {
        ...
        {
            IVirtualProcessorRoot * pVProcRoot = ppVirtualProcessorRoots[i];
            unsigned int maskId = GetResourceMaskId(pVProcRoot->GetExecutionResourceId());
    ...
    /// </summary>
    void SchedulerBase::PerformServiceScan(ULONG serviceTime)
    {
        //
        ...
        for(;;)
        {
            ULONG readServiceTime = m_lastServiceScan;
            ULONG deltaTime = serviceTime - readServiceTime;

            if ((ULONG)InterlockedCompareExchange(reinterpret_cast<volatile LONG *>(&m_lastServiceScan), 
                                                  serviceTime,
