...
namespace details
{
    ...
    public:
        ...
        MultiWaitBlock(size_t waitObjects, bool timeout, bool timer) : 
            m_count(0), m_completions(0), m_fHasTimeout(timeout), m_fWaitTimedOut(false), m_waiters(waitObjects + (timer ? 1 : 0)), m_pSatisfiedBy(NULL), m_hTimer(NULL), m_finalTrigger(0)
        {
        }
...
namespace details
{
    ...
    public:
        ...
        TimedSingleWaitBlock(bool hasTimer) : 
            m_fWaitTimedOut(false), m_signalCounter(0), m_deRef(0), m_eventNode(nullptr), m_hasTimer(hasTimer)
        {
        ...
        bool createTimer(unsigned int timerout)
        {
            if (m_hasTimer)
            {
                ...
                {
                    return CreateTimerQueueTimer(&m_hTimer, 
                                    GetSharedTimerQueue(), 
        ...
        void destroyTimer(bool waitForOutstandingCallback)
        {
            if (m_hasTimer)
            {
                ...
                else
                {
                    for(;;)
                    {
                        if (!DeleteTimerQueueTimer(GetSharedTimerQueue(), m_hTimer, waitForOutstandingCallback ? INVALID_HANDLE_VALUE : NULL))
                        {
