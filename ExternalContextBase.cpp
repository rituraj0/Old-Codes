...
namespace details
{
        ...
        m_pSubAllocator(NULL)
    {
        ...
        m_pStats = _concrt_new ExternalStatistics();
    ...
    void CALLBACK ExternalContextBase::ImplicitDetachHandlerXP(PVOID parameter, BOOLEAN is_timeout)
    {
        ...
        // This is non-blocking
        UnregisterWait(pContext->m_hWaitHandle);
        ...
        if (!explicitAttach)
        {
            ...
            else
            {
                if (!RegisterWaitForSingleObject(&m_hWaitHandle, 
                                m_hPhysicalContext, 
                                ExternalContextBase::ImplicitDetachHandlerXP, 
                                this, INFINITE, (WT_EXECUTEONLYONCE | WT_EXECUTEINWAITTHREAD)))
                {
                    throw scheduler_resource_allocation_error(HRESULT_FROM_WIN32(GetLastError()));
                }
            }
