...
namespace Concurrency
{
    ...
    ///     use the version of the the method that accepts an array of group affinities as a parameter, to restrict affinity on machines with greater
    ...
    /// </remarks>
    /**/
    _CRTIMP void __cdecl set_task_execution_resources(DWORD_PTR dwProcessAffinityMask)
    {
    ...
    /// </remarks>
    /**/
    _CRTIMP void __cdecl set_task_execution_resources(USHORT count, PGROUP_AFFINITY pGroupAffinity)
    {
...
namespace details
{
    ...
    HardwareAffinity::HardwareAffinity(HANDLE hThread)
    {
        ...
        if (::Concurrency::GetOSVersion() >= ::Concurrency::IResourceManager::Win7OrLater)
        {
            ...
            CONCRT_VERIFY(fnDecodedFunction(hThread, &m_affinity));
        }
        else
        {
        ...
        CONCRT_VERIFY(GetThreadGroupAffinity(hThread, &m_affinity));
#endif  // !(defined(_M_IX86) || defined(_M_X64))
    }
    ...
    void HardwareAffinity::ApplyTo(HANDLE hThread)
    {
        ...
        if (::Concurrency::GetOSVersion() >= ::Concurrency::IResourceManager::Win7OrLater)
        {
            ...
            CONCRT_VERIFY(fnDecodedFunction(hThread, &m_affinity, NULL));
        }
        else
        {
            CONCRT_VERIFY(SetThreadAffinityMask(hThread, m_affinity.Mask));
        }
#else   // !(defined(_M_IX86) || defined(_M_X64))
        CONCRT_VERIFY(SetThreadGroupAffinity(hThread, &m_affinity, NULL));
#endif  // !(defined(_M_IX86) || defined(_M_X64))
    }
...
#define THREAD_GROUP_DLL L"kernel32.dll"
    ...
    unsigned int ResourceManager::GetCurrentNodeAndCore(unsigned int * pCore)
    {
        ...
        else
        {
            ...
            // For Vista/Server 2008, we need to compute the node id and core id from the absolute processor id
...
#define LOGICAL_PROCESSOR_DLL L"kernel32.dll"
            ...
            pfnGetLogicalProcessorInformationEx(relationship, NULL, &s_logicalProcessorInformationLength);
#else   // !(defined(_M_IX86) || defined(_M_X64))
            GetLogicalProcessorInformationEx(relationship, NULL, &s_logicalProcessorInformationLength);
#endif  // !(defined(_M_IX86) || defined(_M_X64))
...
#if     (defined(_M_IX86) || defined(_M_X64))
            if (!pfnGetLogicalProcessorInformationEx(relationship, (PSYSTEM_LOGICAL_PROCESSOR_INFORMATION_EX) s_pSysInfo, &s_logicalProcessorInformationLength))
#else   // !(defined(_M_IX86) || defined(_M_X64))
            if (!GetLogicalProcessorInformationEx(relationship, (PSYSTEM_LOGICAL_PROCESSOR_INFORMATION_EX) s_pSysInfo, &s_logicalProcessorInformationLength))
#endif  // !(defined(_M_IX86) || defined(_M_X64))
            {
                throw scheduler_resource_allocation_error(HRESULT_FROM_WIN32(GetLastError()));
            }
    ...
    void ResourceManager::InitializeSystemInformation(bool fSaveTopologyInfo)
    {
...
#endif  // (defined(_M_IX86) || defined(_M_X64))
        {
            ...
            while (byteOffset < s_logicalProcessorInformationLength)
            {
                if (pSysInfoEx->Relationship == RelationProcessorPackage)
                {
                    ...
                    do
                    {
                        ...
                        ++index;
                    } while (index < pSysInfoEx->Processor.GroupCount);
                }
            ...
            ASSERT(s_logicalProcessorInformationLength > 0);
            ASSERT(processorPackageCount > 0 && numaNodeCount > 0);
...

                            {
                                ...
                                do
                                {
                                    if (pSysInfoEx->Processor.GroupMask[group].Mask != 0)
                                    {
                                        ...
                                        m_pGlobalNodes[nodeIndex].Initialize(this, nodeIndex, pSysInfoEx->Processor.GroupMask[group].Group,
                    ...
                    case RelationNumaNode:
                        {
                            ...
                            if (!s_countProcessorPackages)
                            {
                                ...
                                {
                                    // Note that the group affinity mask has already been modified to reflect affinity restrictions placed by process affinity or the user.
                                    m_pGlobalNodes[nodeIndex].Initialize(this, nodeIndex, pSysInfoEx->NumaNode.GroupMask.Group, pSysInfoEx->NumaNode.GroupMask.Mask);
                    ...
                    case RelationProcessorPackage:
                        {
                            ...
                            if (s_countProcessorPackages)
                            {
                                if (pSysInfo->ProcessorMask != 0)
                                {
                                    ...
                                    m_pGlobalNodes[nodeIndex].Initialize(this, nodeIndex, 0, pSysInfo->ProcessorMask);
                                    ++nodeIndex;
                                }
                            }
                        }
        ...
        delete [] m_drmInitialState;
            ...
            for (unsigned int i = 0; i < m_nodeCount; ++i)
            {
                ...
                if (pProcessorGroups != NULL)
                {
                    ...
                    processorGroup = pProcessorGroups[i];
                }

                m_pGlobalNodes[i].Initialize(this, i, processorGroup, processAffinityMask, (unsigned int)pCoreCount[i], baseProcNum);
            }
#if defined(CONCRT_TRACING)
            // Assumes a m x n allocation.
            m_numTotalCores = m_nodeCount * m_pGlobalNodes[0].m_coreCount;
    ...
    unsigned int ResourceManager::ReserveCores(SchedulerProxy * pSchedulerProxy, unsigned int request, unsigned int useCount)
    {
        ...
        if (fAvailableCores)
        {
            ...
            for (unsigned int i = 0; i < m_nodeCount; ++i)
            {
                ...
                ASSERT(pMaxNode->m_availableCores >= 0 && pMaxNode->m_availableCores <= INT_MAX);
